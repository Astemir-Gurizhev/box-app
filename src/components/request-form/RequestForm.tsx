...
export const RequestForm = () => {
	...
	const handleCreateQuestions = async () => {
		...
		const data = await fetchChatResponse(
			russianRequest +
				clearSymbols +
				'Представь, что ты учитель. Какие 3 не связанных друг с другом темы стоило бы предварительно изучить по данной задаче и перечисли названия тем в виде массива [], где каждый ответ является элементом в виде строки "" и разделен запятыми без начального объяснения' +
				task
		)
