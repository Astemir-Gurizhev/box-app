...
export const RequestForm = () => {
	...
	const handleCreateQuestions = async () => {
		...
		const data = await fetchChatResponse(
			russianRequest +
				clearSymbols +
				'Представь, что ты учитель. Какие 3 разные темы стоило бы предварительно изучить по вопросу и создай его в виде массива [], где каждый ответ является элементом в виде строки "" и разделен запятыми без начального объяснения' +
				task
		)
